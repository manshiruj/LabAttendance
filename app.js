...
function setMsg(elId, text, type='') {
  ...
  if (!el) return;
  el.textContent = text;
  el.className = 'msg' + (type ? ' '+type : '') + (elId==='mark-msg' ? ' text-center' : '');
...
function showScreen(name) {
  // hide all screens
  ['screen-verify','screen-admin-login','screen-main'].forEach(id => {
    $(id).style.display = 'none';
  });

  // show admin entry button only on verify screen
  $('admin-entry-wrap').style.display = (name === 'verify') ? 'block' : 'none';
  ...
  return new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        else reject(`You are ${Math.round(dist)}m away from the lab. Must be inside to mark attendance.`);
      },
      err => {
        $('gps-overlay').style.display = 'none';
        const msgs = {1:'Location permission denied. Please allow location access.', 2:'Cannot get location. Try near a window.', 3:'Location timed out. Please try again.'};
...
async function sendOTP(phone) {
  ...
  const msg = `Your LabAttend OTP is ${otp}. Valid 10 min. Do not share.`;
  const url = `https://www.fast2sms.com/dev/bulkV2?authorization=${CONFIG.FAST2SMS_KEY}&route=q&message=${encodeURIComponent(msg)}&language=english&flash=0&numbers=${phone}`;
  ...
  try {
    ...
    // If CORS blocked, try no-cors (SMS still sends, we just can't read response)
    try {
      await fetch(url, { method: 'GET', mode: 'no-cors' });
      return otp; // assume sent
    } catch {
      throw new Error('Could not send OTP. Check your internet connection.');
    }
  }
}
...
$('btn-send-otp').addEventListener('click', async () => {
  ...
  setMsg('verify-msg', '');
  if (!/^\d{10}$/.test(phone)) { setMsg('verify-msg','Enter a valid 10-digit number.','error'); return; }

  const member = getMembers().find(m => m.phone === phone);
  if (!member) { setMsg('verify-msg','This number is not registered. Contact your lab admin.','error'); return; }
...
$('btn-verify-otp').addEventListener('click', () => {
  ...
  setMsg('otp-msg','');
  if (!pendingOTP)         { setMsg('otp-msg','OTP expired. Request a new one.','error'); return; }
  if (entered !== pendingOTP) { setMsg('otp-msg','Incorrect OTP. Try again.','error'); return; }
...
$('btn-back-to-verify').addEventListener('click', () => showScreen('verify'));
...
function unlockAdmin() {
  if ($('admin-login-pass').value === CONFIG.ADMIN_PASS) {
    ...
    renderBoard();
    // open admin overlay immediately
...
function initMainScreen() {
  ...
  if (!device) { showScreen('verify'); return; }

  const member = getMembers().find(m => m.id === device.memberId);
  if (!member) { store.del('la_device'); showScreen('verify'); return; }
...
async function markAttendance(type) {
  ...
  if (!device) { showScreen('verify'); return; }
  const member = getMembers().find(m => m.id === device.memberId);
  if (!member) { setMsg('mark-msg','Not registered. Contact admin.','error'); return; }

  const tod = today(), records = getRecords();
  const existing = records.find(r => r.id === member.id && r.date === tod);

  if (type==='in'  && existing)      { setMsg('mark-msg',`Already checked in at ${existing.in}.`,'error'); return; }
  if (type==='out' && !existing)     { setMsg('mark-msg','You haven\'t checked in today yet.','error'); return; }
  if (type==='out' && existing?.out) { setMsg('mark-msg',`Already checked out at ${existing.out}.`,'error'); return; }

  try { await checkGPS(); }
  catch(err) { setMsg('mark-msg', err, 'error'); return; }

  const time = nowTime();
  if (type==='in') records.push({id:member.id, name:member.name, role:member.role, date:tod, in:time, out:null});
  else existing.out = time;
  setRecords(records);

  setMsg('mark-msg', `✓ ${type==='in'?'Check-in':'Check-out'} recorded at ${time}`, 'success');
  updateMcardStatus(member);
  renderBoard();
}
...
function renderBoard() {
  const members = getMembers(), records = getRecords();
  ...
  if (!members.length) {
    ...
    ['stat-present','stat-absent','stat-total'].forEach(id => $(id).textContent='0');
    return;
  }
...
$('admin-btn').addEventListener('click', () => {
  if (!adminUnlocked) {
    // send admin to login screen from main
    showScreen('admin-login');
...
$('close-overlay').addEventListener('click', () => { $('admin-overlay').style.display = 'none'; });
$('admin-overlay').addEventListener('click', e => { if(e.target===$('admin-overlay')) $('admin-overlay').style.display='none'; });
...
function addMember() {
  ...
  const roleV = $('new-role').value;
  const role  = roleV==='__custom__' ? $('new-role-custom').value.trim() : roleV;
  const errEl = $('add-err');
  ...
  if (members.find(m => m.phone===phone))                                        { errEl.textContent='Phone already registered.'; return; }
  if (members.find(m => m.name.toLowerCase()===name.toLowerCase()))              { errEl.textContent='Name already exists.'; return; }

  members.push({id:uid(), name, phone, role});
  setMembers(members);
  $('new-name').value=$('new-phone').value=$('new-role').value=$('new-role-custom').value='';
...
function renderMemberList() {
  ...
  ul.innerHTML = '';
  $('member-empty').style.display = members.length ? 'none' : 'block';
  members.forEach(m => {
...
$('member-list').addEventListener('click', e => {
  ...
  if (!member || !confirm(`Remove ${member.name}? Records kept.`)) return;
